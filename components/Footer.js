export default function Footer() {
    return(
        <footer className=" dark:bg-gray-600 bg-black md:h-[-300px]  md:w-[100%] text-amber-100 clip-path:eli" >
         <div className="flex md:flex-row ">
            <img src="https://restan-nextjs.vercel.app/_next/image?url=%2Fassets%2Fimg%2Fshape%2F9.png&w=1920&q=75" className=' absolute  md:h-[150px] md:w-[150px] mx-0'/>
             
            <div className=" text-center md:width-[full] md:p-10 text-white align-baseline ">
                   <div className=" md:h-[400px] md:w-[1250px] w-[1350px] md:mx-[80px] relative flex md:flex-row  md:mt-[-190px] items-center dark:bg-gray-900 bg-black  ">
                <div className="flex md:flex-row flex-col">
                    <div className="flex flex-col">
                         <div className="flex flex-row">
                        <div className="text-1xl md:p-1 p-4 ">
                            <div className="text-2xl font-serif p-1">
            About Us </div>
           <div className="md:text-1xl text-sm md:p-1 ">
           Continued at zealously necessary is </div>
            <div className="md:text-1xl text-sm p-1 ">
           Surrounded sir motionless she end</div>
            <div className="md:text-1xl text-sm p-1 ">
           literature. Gay direction neglected.
            </div>
            <div className="flex flex-row md:mt-5 mt-1 ">
                   <img src = "data:image/png;base64,"
                  className = " h-10 w-10 ml-6 rounded-[5px]" alt = "fb"
                ></img>
                <img src = "https://assets1.chainstoreage.com/images/v/max_width_1440/2023-07/twitter-x-logo.png"
                  className = " h-10 w-10 ml-6 " alt = "tw"
                ></img>
                 <img src = "data:image/png;base64,"
                  className = " h-10 w-10 ml-6 " alt = "yt"
                ></img>
                <img src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQfIZNwih6eXIffaQEDckh1GtVTsxxkkmMvo4keYRVLAg&s"
                  className = " h-10 w-10 ml-6 rounded-[10px]" alt = "l-in"
                ></img>



           
           
           </div>
           
            </div> 
            </div>
                    </div>
               <div className=" hidden md:block w-px h-[250px] bg-white ml-[60px]"></div>

                    <div className="flex flex-col md:mx-[40px] mx-[100px]">
                        <div className='flex flex-row'>
                              <div className="flex flex-col ">
                             <div className="text-2xl font-serif text-semibold p-1 ">
            Explore
            </div> 
             <div className="text-1xl md:p-4 ">
            Our Home
            </div>  <div className="text-1xl p-1 ">
           Contact US
            </div>  <div className="text-1xl p-1 ">
            Reviews 
            </div> 


                        </div>
                           <div className="flex  flex-col md:mx-[40px]">
                        <div className='flex md:flex-row flex-col'>
                              <div className="flex hidden md:block flex-col">
                                
              <div className = "font-bold text-[30px] text-white ml-0  mb-10 flex flex-col">Contact Info</div>
                <div className = "text-white text-[17px] mb-5 -mt-5 w-70">1234 - Bandit Tringi St, Sonipat, Haryana</div>
                <div className = "text-white text-[17px] mb-5">Phone: +91 1234567890</div>
                <div className = "text-white text-[17px] mb-5">Email: food@restan.com </div>
            </div></div>
            </div>
             
            
                           <div className="flex flex-col hidden md:block md:mx-[40px] mx-0">
                        <div className='flex  md:flex-row'>
                              <div className="flex flex-col">
                <div className = "font-bold text-[30px] text-white  md:mt-20   mb-10 flex flex-col">Newsletter</div>
                <div className = "text-white text-[17px]  mb-5 -mt-5 cursor-pointer hover:text-yellow-400">Join our subscribers list to get the latest news and special offers. </div>
                <input type = "Email" placeholder="Your Email Address" className = "w-60 h-10 mt-5 -ml-5 rounded-[5px] pl-2 border-b-1 border-b-gray-300 text-[16px]">
                </input>
            </div>    </div>    </div>    


             
                            </div>
                    </div>
                </div>
            </div>
                <div className="flex flex-row md:flex-col">
            <div className="flex md:flex-row flex-col">
              
            <img src="https://restan-nextjs.vercel.app/_next/image?url=%2Fassets%2Fimg%2Flogo-light.png&w=1920&q=75" className='h-[70px] w-[150px] md:mx-[120px]  '/>
            <div className="md:text-2xl md:p-10 p-0 md:mt-0 mt-20 font-serif ">
            Copy rights @Preethamdas @Prathameshvshenoy 
            </div>
             <div className="md:text-2xl md:p-10 md:p-30 font-serif "></div>
            </div>
            </div>
            </div>
            </div>
        </footer>
    );
}